...
public:
	...
	void update_loiter_3d(const struct Location &anchor, const struct Location &center_WP, float radius, float psi, float theta, float w, float sigma, int32_t dist, int8_t loiter_direction, Matrix3f M_pe, int32_t segment, int32_t &height);
	void update_winddrone(Vector3f normal_vec, float sphere_radius, float circle_radius, struct Location sphere_center, float distance, float spring_const, int8_t loiter_direction);
...
private:
    ...
    // commanded roll angle, just for data sampling, mutable so it can be modified in the const function nav_roll_cd()
    mutable float _roll_dem;
