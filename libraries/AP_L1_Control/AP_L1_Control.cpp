...
void AP_L1_Control::update_loiter(const struct Location &center_WP, float radius, int8_t loiter_direction)
{
    ...
    float latAccDemCircPD = (xtrackErrCirc * Kx + xtrackVelCirc * Kv);  //original
    //float latAccDemCircPD = (xtrackErrCirc * Kx + xtrackVelCirc * Kv - Kx*11.9);	//my change
    ...
    //float latAccDemCircCtr = velTangent * velTangent / MAX((0.5f * radius), (radius + xtrackErrCirc)); //original
    float latAccDemCircCtr = velTangent * velTangent / radius;   //my change
	...
	*/
    ...
    f << now << " " << A_air.x << " " << A_air.y << " " << _current_loc.alt << " " << _ahrs.roll_sensor << " " << _ahrs.pitch_sensor << " " << _ahrs.yaw_sensor
    		 << _latAccDem << endl;
    f.close();*/
}
...
void AP_L1_Control::update_loiter_3d(const struct Location &anchor, const struct Location &center_WP, float radius, float psi, float theta, float w, float sigma, int32_t dist, int8_t loiter_direction, Matrix3f M_pe, int32_t segment, int32_t &height)
{
    ...
    float spring_const;
    if (_position_vec_ef.length() > 490) {
    	spring_const = 2;
    } else {
    	spring_const = 0;
    }
    Vector3f tether_tension = _position_vec_ef.normalized()*(spring_const*(500 - 490)*4/7.785);
    float tetherErr = - sin_chi*tether_tension.x + cos_chi*tether_tension.y;

    //end of error calculation
    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    hal.console->print("xtrackErrCirc: ");
    hal.console->println(-loiter_direction*xtrackErrCirc);
    ...
    //float latAccDemCircCtr = velTangent * velTangent / MAX(0.5 * radius, radius) * cos_clear / sqrtf(A_air_unit.y*A_air_unit.y*cos_slope*cos_slope + A_air_unit.x*A_air_unit.x); // y Komponente von a im bahnfesten Koordinatensystem
...
*/
    ...
    float latAccDemCirc = (latAccDemCircPD + loiter_direction * latAccDemCircCtr + tetherErr);
